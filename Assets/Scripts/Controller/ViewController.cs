...
public class ViewController : MonoBehaviour
{
    ...
    void RoamingMode()
    {
        ...
        Vector3 posX = transform.right * h * roamingSpeed*Time.deltaTime;
        Vector3 posZ = transform.forward * v * roamingSpeed * Time.deltaTime;
        Vector3 tempPos = posX + posZ;

        if (Input.GetKey(KeyCode.Q))
        {
            transform.position += new Vector3(0, roamingSpeed * Time.deltaTime, 0);
        }
        ...
        transform.position += new Vector3(tempPos.x,0,tempPos.z);
    }
    void MouseEvents()
    {
        //按住鼠标右键旋转
